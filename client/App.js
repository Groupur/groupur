...
class App extends Component {
  constructor(props) {
    ...
    this.addAllGroups = this.addAllGroups.bind(this);
  ...
  combiner(e){
    ...
    this.addGroup(e,value)
    this.addAllGroups(e,value)

  }

  removeGroup(i){
    let newArr = [...this.state.groupArray]
    let value = newArr[i]; 
    newArr = newArr.slice(0,i).concat(newArr.slice(i+1))
    this.setState({groupArray: newArr})
...
// TotalArray -> for all available groups
  addAllGroups(e,value){
    e.preventDefault();
    ...
    .then(data => {
      this.setState({totalArray: data})
      console.log('TotalGroups' , this.state)
    })
  }

// user array -> GroupArray
  addGroup(e,value){
    e.preventDefault();
    ...
    .then(data => {
      ...
      console.log('UserArray' , this.state)
    })
  }
  ...
  render() {      
    return (
      ...
      this.state.activeSession ? <MainPage removeGroup={this.removeGroup} combiner={this.combiner} groupArray={this.state.groupArray} totalArray={this.state.totalArray}/> : <Login handleClick={this.handleClick}/>
    );
  }
}
